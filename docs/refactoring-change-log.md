...
| `Wish` | Wish가 `Product` 객체를 직접 들고, 회원은 `memberId` 값으로만 참조했다. | Wish를 별도 Aggregate root로 삼을 때 Product 객체 참조가 Product Aggregate와의 경계를 흐렸고, 조회 후 저장 방식의 중복 추가는 동시 요청에서 중복 row를 만들 수 있었다. | Wish는 `memberId`, `productId` 값만 보관한다. 상품 정보가 필요한 응답 조립은 Wish UseCase 서비스에서 `ProductRepository`로 조회한다. 같은 회원이 같은 상품을 동시에 위시하면 `(member_id, product_id)` unique 제약으로 하나의 row만 남긴다. 첫 insert 성공은 `200 OK`, unique 위반은 `400 Bad Request`와 `이미 위시한 상품입니다.` 메시지로 처리한다. | `WishContractTest`, `WishServiceTest`, `WishApiTest`, `V12__Add_wish_member_product_unique_constraint.sql` |
...
- [x] 중복 Wish 추가는 `(member_id, product_id)` unique 제약으로 하나만 저장하고, unique 위반은 `400 Bad Request`로 처리한다.
- [ ] 소유권 기반 삭제 경쟁을 검토한다.
